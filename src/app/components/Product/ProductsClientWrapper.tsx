...
const CategoryBreadcrumb = ({ categoryId, categories }: { categoryId: number; categories: any[] }) => {
  ...
  return (
    ...
    >
      <Link href={"/"} className="text-[11px] hover:text-[#ff6900]" itemProp="name">
        Home
      </Link>
      {path?.map((cat: any, index: number) => (
        <span key={cat.id}>
          ...
          {index === path.length - 1 ? (
            <span className="text-[#ff6900]">{cat.name}</span>
          ) : (<Link href={`/category/${cat?.slug}`}
            className={`text-[11px] text-[#666666]  hover:text-[#ff6900]  transition-colors`}
          >
  ...
  return (
    ...
    >
      ...
      <span>
        ...
        <span className="text-[#ff6900] cursor-pointer">{brandName}</span>
      </span>
    </nav>
  );
};
