        ...
        <section className="mb-8">
            ...
            <ul className="list-disc list-inside ml-4 space-y-1">
                {visiblePages?.map((page: any, i: number) => (
                    <li key={i}>
                        {page?.pageType == "2" ? (
                            <Link href={page?.link || "#"} target="_blank" rel="noopener noreferrer" className="text-[#ff6900] text-[14px] underline">
                                {page?.pageName}
                            </Link>
                        ) : (
                            <Link href={page?.slugWithUrl || page?.pageUrl || "#"} className="text-[#ff6900] text-[14px] underline">
                                {page?.pageName}
                            </Link>
                        )}
                    </li>
