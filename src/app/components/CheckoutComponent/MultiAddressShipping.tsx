            ...
            <DialogContent className="sm:max-w-[640px] bg-white p-0 overflow-hidden">
                ...
                <div className="px-6 py-3 bg-gray-50 border-b flex justify-between items-center">
                    ...
                    <div className="flex gap-4">
                        <button type="button" onClick={() => { const c: Record<string, number> = {}; cart.forEach((i) => { c[String(i.id)] = 0; }); setQuantities(c); }} className="text-[16px] text-[#eb6302] hover:underline">Clear all</button>
                        <button type="button" onClick={() => { const a: Record<string, number> = {}; cart.forEach((i) => { a[String(i.id)] = getLeft(i.id); }); setQuantities(a); }} className="text-[16px] text-[#eb6302] hover:underline">Select all items left</button>
                    </div>
                </div>
                        ...
                        <div className="px-4 pb-4 space-y-3">
                            {dest.address ? (
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button type="button" onClick={() => openAddressModal(dest.id)} className="text-[1rem] text-[#eb6302] hover:underline">Enter shipping address</button>
                                </div>
                            )}
                                ...
                                <>
                                    {totalAllocatedSlots > 0 ? (
                                        <div>
                                            <div className="flex items-center justify-between">
                                                ...
                                                <div className="flex gap-4 text-[1rem]">
                                                    <button type="button" onClick={() => dispatch(toggleShowItems(dest.id))} className="text-xs text-[#eb6302] hover:underline">
                                                        {dest.showItems ? "Hide items ▲" : "Show items ▼"}
                                                    </button>
                                                    <button type="button" onClick={() => { setEditingDestId(dest.id); setAllocateModalOpen(true); }} className="text-xs text-[#eb6302] hover:underline">
                                                        Reallocate items
                                                    </button>
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button type="button" onClick={() => { setEditingDestId(dest.id); setAllocateModalOpen(true); }} className="text-[1rem] text-[#eb6302] hover:underline">Allocate items</button>
                                        </div>
                                    )}
