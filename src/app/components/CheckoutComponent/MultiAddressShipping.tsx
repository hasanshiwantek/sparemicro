      ...
      <DialogContent className="sm:max-w-[560px] max-h-[90vh] overflow-y-auto bg-[#eeeeee]">
        ...
        <div className="space-y-4 pt-2">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-[16px] text-gray-700 mb-1 block">
                First Name <span className="text-[#ff6900]">*</span>
              </label>
              <Input
                ...
                className={`h-[42px] bg-white ${errors.firstName ? "border-red-500" : ""}`}
              />
              {errors.firstName && (
                <p className="text-xs text-[#ff6900] mt-1">{errors.firstName}</p>
              )}
            </div>
            <div>
              <label className="text-[16px] text-gray-700 mb-1 block">
                Last Name <span className="text-[#ff6900]">*</span>
              </label>
              <Input
                ...
                className={`h-[42px] bg-white ${errors.lastName ? "border-red-500" : ""}`}
              />
              {errors.lastName && (
                <p className="text-xs text-[#ff6900] mt-1">{errors.lastName}</p>
              )}
            </div>
          </div>
          <div>
            <label className="text-[16px] text-gray-700 mb-1 flex justify-between">
            ...
            <Input
              ...
              className="h-[42px] bg-white max-w-full"
            />
          </div>
          <div>
            <label className="text-[16px] text-gray-700 mb-1 flex justify-between">
          ...
          <div>
            <label className="text-[16px] text-gray-700 mb-1 block">
              Address Line 1 <span className="text-[#ff6900]">*</span>
            </label>
            <Input
              value={value.address1}
              onChange={(e) => {
                onChange({ ...value, address1: e.target.value });
                setErrors((p) => ({ ...p, address1: "" }));
              }}
              className={`h-[42px] bg-white max-w-full ${errors.address1 ? "border-red-500" : ""}`}
            />
            {errors.address1 && (
              <p className="text-xs text-[#ff6900] mt-1">{errors.address1}</p>
            )}
          </div>
          <div>
            <label className="text-[16px] text-gray-700 mb-1 flex justify-between">
          ...
          <div>
            <label className="text-[16px] text-gray-700 mb-1 block">
              City <span className="text-[#ff6900]">*</span>
            </label>
            <Input
              value={value.city}
              onChange={(e) => {
              ...
              className={`h-[42px] bg-white max-w-full ${errors.city ? "border-red-500" : ""}`}
            />
            {errors.city && (
              <p className="text-xs text-[#ff6900] mt-1">{errors.city}</p>
            )}
          </div>
          {/* <div>
                        <label className="text-[16px] text-gray-700 mb-1 block">City <span className="text-[#ff6900]">*</span></label>
                        ...
                        {errors.city && <p className="text-xs text-[#ff6900] mt-1">{errors.city}</p>}
          ...
          <div>
            ...
            <Select
              value={value.country}
              onValueChange={(v) =>
                onChange({ ...value, country: v, state: "", city: "", zip: "" })
              }
            >
              <SelectTrigger className="h-[42px] bg-white max-w-full">
            ...
            {errors.country && (
              <p className="text-xs text-[#ff6900] mt-1">{errors.country}</p>
            )}
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              {stateList.length > 0 ? (
                ...
                <Input
                  ...
                  className={`h-[42px] bg-white max-w-full ${errors.state ? "border-red-500" : ""}`}
                />
              )}
              {errors.state && (
                <p className="text-xs text-[#ff6900] mt-1">{errors.state}</p>
              )}
            </div>
            <div>
              <label className="text-[16px] text-gray-700 mb-1 block">
              ...
              />
              {errors.zip && (
                <p className="text-xs text-[#ff6900] mt-1">{errors.zip}</p>
              )}
            </div>
          </div>
...
const AllocateModal: React.FC<{
  ...
  useEffect(() => {
    ...
    cart?.forEach((item) => {
      init[String(item.id)] =
        dest?.allocatedItems.filter((s) => s.split("-")[0] === String(item.id))
          .length || 0;
    });
    setQuantities(init);
      ...
      <DialogContent className="sm:max-w-[640px] bg-white p-0 overflow-hidden">
        ...
        <div className="px-6 py-3 bg-gray-50 border-b flex justify-between items-center">
          ...
          <div className="flex gap-4">
            <button
              ...
              className="text-[16px] text-[#ff6900] hover:underline"
            >
              Clear all
            </button>
            <button
        ...
        <div className="overflow-y-auto max-h-[320px]">
          {cart.map((item: any) => {
            ...
            const img = item.image?.[0]?.path || item.image?.[0]?.url;
            return (
              <div
                key={String(item.id)}
                className="grid grid-cols-12 gap-2 px-6 py-4 border-b items-center"
              >
                ...
                <div className="col-span-3 text-center text-[16px] text-gray-600">
                  {left}
                </div>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t flex justify-end gap-3">
          <button
            type="button"
            onClick={onClose}
            className="px-6 py-2 text-[16px] border border-gray-300 rounded text-gray-600 hover:bg-gray-100"
          >
            CANCEL
          </button>
          <button
            ...
            className="px-6 py-2 text-[16px] bg-gray-200 text-gray-600 rounded hover:bg-gray-300 font-medium uppercase"
          >
            ALLOCATE
          </button>
        </div>
...
const MultiAddressShipping = ({
  ...
  const [destShippingRates, setDestShippingRates] = useState<
    Record<string, any[]>
  >({});
  const [destRatesLoading, setDestRatesLoading] = useState<
  ...
  const canContinue = useMemo(
    () =>
      ...
      destinations.every(
        (d) =>
          d.address && d.allocatedItems.length > 0 && d.selectedShippingMethod,
      ),
  ...
  const allocateItems = (
    allocations: { itemId: string | number; qty: number }[],
  ) => {
    if (!editingDestId) return;
    const updatedDestinations = destinations.map((d) => {
      if (d.id === editingDestId) {
        const withoutThese = d.allocatedItems.filter(
          (slot) =>
            !allocations.some((a) => slot.split("-")[0] === String(a.itemId)),
        );
  ...
  const defaultRates: ShippingRate[] = [
    {
      ...
      service_name: "",
      display_name:
        "Ship on my own/company shipping account (mention shipping account on order comments)",
      total_charge: 0,
      is_fedex: false,
    },
    {
      ...
      service_type: "flat",
      service_name: "",
      display_name: "Flat Rate for under 10 LBS*",
      total_charge: 10,
      is_fedex: false,
    },
  ];
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="px-4 pb-4 space-y-3">
              {dest.address ? (
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => openAddressModal(dest.id)}
                    className="text-[1rem] text-[#ff6900] hover:underline"
                  >
              ...
              {dest.address && (
                <>
                  {totalAllocatedSlots > 0 ? (
                    <div>
                      <div className="flex items-center justify-between">
                        <span className="text-[1rem] font-bold text-gray-800">
                          {totalAllocatedSlots} item
                          {totalAllocatedSlots > 1 ? "s" : ""} allocated
                        </span>
                        <div className="flex gap-4 text-[1rem]">
                          <button
                            type="button"
                            onClick={() => dispatch(toggleShowItems(dest.id))}
                            className="text-xs text-[#ff6900] hover:underline"
                          >
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-[1rem] font-bold text-gray-700">
                        No item allocated
                      </span>
                      <button
                        type="button"
                  ...
                  {totalAllocatedSlots > 0 && (
                    <div>
                      <h5 className="text-[1rem] font-bold text-gray-800 mb-2">
                        Shipping Method
                      </h5>
                        ...
                        <div className="space-y-2">
                          {activeRates.map((rate: any) => (
                            <label
                              ...
                              className="flex items-center gap-2 text-[1rem] text-gray-700 cursor-pointer"
                            >
                              <input
                                type="radio"
      ...
      {!lastDestComplete && destinations.length > 0 && (
        <p className="text-[13px] text-[#ff6900] flex items-center gap-1">
          <span>✕</span>
          Please complete the address, item allocation, and method selection for
          Destination #{destinations.length}.
        </p>
      )}
      <div className="flex flex-col">
        <label className="text-[16px] font-medium text-gray-700 mb-1">
          Order Comments
        </label>
        <textarea
          rows={2}
