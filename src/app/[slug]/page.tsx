...
export async function generateMetadata({
  ...
  const url = `https://sparemicro.vercel.app/${slug}`;

  if (webPages) {
    return {
      title: `${webPages.pageTitle || webPages.pageName} | Spare Micro`,
      description:
  ...
  return {
    title: `${product.pageTitle || product.name} | Spare Micro`,
    description:
      product.metaDescription?.substring(0, 160) ||
      product.description?.substring(0, 160) ||
      "Buy quality servers, networking equipment, and IT solutions at Spare Micro.",
    keywords:
      product.searchKeywords ||
      `${product.name}, ${product.brand?.name}, Spare Micro`,
    alternates: {
      canonical: url,
    },
    openGraph: {
