  ...
  return (
    <ul className="list-disc list-inside ml-4 space-y-1">
      {visibleCategories.map((cat) => (
        <li key={cat.slug}>
          <Link href={`/category/${cat.slug}`} className="text-[#ff6900] text-[14px] underline">
            {cat.name}
          </Link>
      ...
      {categoriesLength !== undefined && categoriesLength > SHOW_CATEGORIES_LIMIT && (
        <li style={{ listStyleType: "circle" }}>
          <Link href="/sitemap/categories" className="text-[#ff6900] text-[14px] underline">
            Show All
          </Link>
        </li>
      )}
    ...
    <ul className="list-disc list-inside ml-4 space-y-1">
      {visibleBrands.map((brand: any) => (
        <li key={brand.slug} style={{ listStyleType: "circle" }}>
          <Link href={`/brand/${brand.brand.slug}`} className="text-[#ff6900] text-[14px] underline">
            {brand.brand.name}
          </Link>
        </li>
      ))}
      {brands.length > SHOW_BRANDS_LIMIT && (
        <li style={{ listStyleType: "circle" }}>
          <Link href="/sitemap/brands" className="text-[#ff6900] text-[14px] underline">
            Show All
          </Link>
        </li>
      )}
        ...
        <section className="mb-8">
          ...
          <ul className="list-disc list-inside ml-4 space-y-1">
            {webPages.map((page: any, i: number) => (
              <li key={i}>
                <Link href={page?.pageUrl} className="text-[#ff6900] text-[14px] underline">
                  {page?.pageName}
                </Link>
              </li>
            ))}
